...
def upload_files():
    if request.method == 'POST':
        ...
        if selected_module == 'doku':
            ...
            file1_files = request.files.getlist('file1')  
            ...
            try:
                ...
                UploadedFile.query.filter_by(user_id=current_user.id, filename=file2_filename).delete()  # Remove old file entry
                for path in file1_paths:
                    UploadedFile.query.filter_by(user_id=current_user.id, filename=os.path.basename(path)).delete()  # Remove old file entry
            ...
            try:
                UploadedFile.query.filter_by(user_id=current_user.id, filename=file1_filename).delete()  # Remove old file entry
                UploadedFile.query.filter_by(user_id=current_user.id, filename=file2_filename).delete()  # Remove old file entry
